...
const handleMcpRequest = async (req: Request, res: Response, providerSlug?: string) => {
  const token = extractToken(req)
  const organizationIdHeader = req.headers['x-organization-id'] as string
  
  // Log incoming headers for debugging
  logger.info(`MCP Request headers: x-api-key=${req.headers['x-api-key']}, authorization=${req.headers['authorization']?.substring(0, 20)}..., x-secret-token=${req.headers['x-secret-token']}`)
  ...
  if (agent) {
    ...
    logger.info(`[Agent: ${agent.name}] Connected to organization ID: ${organizationId}`)
  ...
  if (providerSlug) {
    ...
    if (provider && token !== provider.apiKey) {
      return res.status(401).json({ error: 'Unauthorized - Invalid API key for provider' })
    }
  } else {
  ...
  logger.info(`[${provider.name}] Connected Agent to organization ID: ${organizationIdHeader}`)
