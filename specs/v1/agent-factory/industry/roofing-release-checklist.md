# Roofing Agent Release Checklist (v1.0.0)

Use this checklist for every new or updated production Roofing agent.

## 1. Identity And Prompt

- [ ] Industry pack `specs/v1/agent-factory/industry/roofing.md` selected.
- [ ] Blueprint `specs/v1/agent-factory/industry/roofing-blueprint.v1.yaml` selected.
- [ ] Compiled prompt `specs/v1/agent-factory/industry/roofing-system-prompt.v1.md` selected.
- [ ] Core-tabs profile `specs/v1/agent-factory/core-tabs/roofing-core-tabs-profile.v1.yaml` selected.
- [ ] Workflow profile `specs/v1/agent-factory/industry/roofing-workflow.v1.yaml` selected.
- [ ] System prompt template ID and Roofing service fragments attached.
...
- [ ] Roofing pages included (`/services`, `/roof-repair`, `/storm-damage`, `/roof-replacement`).
- [ ] Crawl completed without fatal errors.
- [ ] KB ingestion completed.
- [ ] Retrieval smoke test passed for warranty and insurance policy messaging.
...
- [ ] Required Roofing fields configured (`urgency_level`, `service_type`, `issue_summary`, `service_address`).
- [ ] Evaluation criteria configured with Roofing thresholds.
- [ ] Thresholds approved by admin.

## 6. Workflow And Branches

- [ ] Intent map configured for active leak, storm damage, repair, and estimate flows.
